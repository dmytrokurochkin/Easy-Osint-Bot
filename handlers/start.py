...
async def cb_main_menu(callback: CallbackQuery, admin_id: int) -> None:
    ...
    await callback.message.edit_text("Обери дію:", reply_markup=get_main_menu_keyboard(is_admin))
...
async def cb_admin_menu(callback: CallbackQuery, conn, admin_id: int) -> None:
    ...
    await callback.message.edit_text(
        "Налаштування:", reply_markup=get_admin_menu_keyboard(access_mode, ghunt_enabled)
    )
    await callback.answer()
...
async def cb_reports_list(callback: CallbackQuery) -> None:
    ...
    if not reports:
        ...
        return
    await callback.message.edit_text(
        "Твої звіти:", reply_markup=get_reports_list_keyboard(reports)
    )
    await callback.answer()
